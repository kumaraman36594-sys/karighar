  ...
  return (
    <div className="relative inline-flex items-center">
      <button
        ...
        className={`inline-flex items-center justify-center rounded-2xl font-bold cursor-pointer transition-all select-none shadow-sm ${
          isListening
      ...
      {isListening && (
        <div className="absolute right-0 top-full mt-2 z-50 bg-gray-900 text-white px-3 py-2 rounded-xl text-xs shadow-2xl flex items-center gap-2 whitespace-nowrap border border-amber-400/50">
      ...
      {showPresetsMenu && presets && presets.length > 0 && (
        <div className="absolute right-0 top-full mt-2 z-50 w-64 bg-white rounded-2xl shadow-2xl border-2 border-indigo-200 p-3 animate-fade-in">
          ...
          <div className="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto">
            {presets.map((preset, idx) => (
              <button
                ...
                className="px-2.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-800 rounded-lg text-xs font-medium border border-indigo-200 text-left transition-colors cursor-pointer"
              >
                {preset}
              </button>
            ))}
